...
def Train_Pairwise_Full_Split_stepupdate(data,n_features, theta,W,epochs,alpha_actor,alpha_critic,gamma,seed,trainprop):
    """Trains dataset for given number of epochs using W,theta as a starting point"""
    ...
    for i in range(1,epochs+1):
        ...
        for q in train_queries_list:
            ...
            effective_length=min(len_episode,M-1)
            #Episode loop
            while(t!=min(len_episode+1,M-1)):

                #Initial Action 
                a1=sample_action(theta,s,data,q)
                a2=sample_action(theta,s,data,q)
                
                #Get reward
                r1=get_reward(a1,t,data,q)
                r2=get_reward(a2,t,data,q)

                #Compute scorefunctions
                grad_s_a1=compute_scorefn(s,a1,theta,data,q)
                grad_s_a2=compute_scorefn(s,a2,theta,data,q)
                ...
                #Compute Q-Values
                q_s_a1=compute_qvalue(W,psi_s_a1)#Compute the q(s,a)
                q_s_a2=compute_qvalue(W,psi_s_a2)
                ...
                #Next action according to next states(for td error)
                a_next_s_next_a1=sample_action(theta, s_next_a1, data, q)
                a_next_s_next_a2=sample_action(theta, s_next_a2, data, q)

                #scorefns of the actions
                grad_s_next_a1=compute_scorefn(s_next_a1,a_next_s_next_a1,theta,data,q)
                grad_s_next_a2=compute_scorefn(s_next_a2,a_next_s_next_a2,theta,data,q)

                #psi feature vectors
                psi_s_next_a1=grad_s_next_a1
                psi_s_next_a2=grad_s_next_a2

                #Qvalues of the next state and next action
                q_s_next_a1=compute_qvalue(W, psi_s_next_a1)
                q_s_next_a2=compute_qvalue(W, psi_s_next_a2)

                #td errors of a1 and a2
                td_error_a1=r1+gamma*q_s_next_a1-q_s_a1
                td_error_a2=r2+gamma*q_s_next_a2-q_s_a2

                #Compare the estimated G values
                if(q_s_a1>q_s_a2):
                    a=a1
                    grad_s_a=grad_s_a1
                    q_s_a=q_s_a1
                ...
                #Updates
                theta+=alpha_theta*(q_s_a1-q_s_a2)*(grad_s_a1-grad_s_a2) #Policy Update
                W+= alpha_w*(td_error_a1-td_error_a2)*(psi_s_a1-psi_s_a2) #Critic Update
