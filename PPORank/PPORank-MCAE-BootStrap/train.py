...
def train(datadir,n_features,n_iterations,max_ep_len,save_model_freq,update_timestep,K_epochs,eps_clip,gamma,lr_actor,lr_critic,hidden_nodes,random_seed):
    ...
    ################### checkpointing ###################
    #Get dataset name and construct the filename
    ds=get_name(datadir)
    spath=f"./Results/{ds}/{ds}-i_{n_iterations}-lr_ac_{lr_actor},{lr_critic}-g_{gamma}-hnodes_{hidden_nodes}-up_T_{update_timestep}-max_T_{max_ep_len}_epochs_{K_epochs}-seed_{random_seed}"
...
if __name__ == '__main__':
    ...
    ap.add_argument("-steps", "--episode_length", required=False, default=999999,
