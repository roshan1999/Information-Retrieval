...
def import_all(filename):
    """Imports dataset and does a 70-30 split into a structure:{qid:{docid:(features,label),docid2:(features_2,label_2)},qid2:{docid:(),docid2:()}} and returns train,test dictionarys. """
    f = open(filename, 'rb')
    t = pickle.load(f)
    s = pd.Series(t)
