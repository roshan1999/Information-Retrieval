...
def validate_individual(DOC_TRUTH, MAX_DCG, doc_action_list):
    """Based on the actions taken;returns ndcg scores from position 1 to 10 """
    label = []
    results = []
...
def calculate(dcg_results):  # dcg_results = {Q: [NDCG_scores]*10}
    """Given ndcg values for each query;averages across to get mean ndcg scores for current epoch"""
    final = [0] * 10
    counter = 0
