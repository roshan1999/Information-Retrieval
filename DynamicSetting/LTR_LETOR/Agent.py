...
def sample_episode_Categorical(W, concat_fv):

    M = len(list(concat_fv)) # Length of the list of documents
    new_doc_list = []

    doc_list = list(concat_fv) # Dict of {docIds: fv} for query Q
    ...
    for t in range(M):
        ...
        for docId in doc_list:
            features_list.append(concat_fv[docId])
...
def rank(curr_doc_list, curr_query_id, W, data):

    # Generate each document's feature vectors
    # doc_fv: {docID: [feature vectors of doc]}
    doc_fv = {}
    all_fv = data.getFeatures()
    for doc in curr_doc_list:
        doc_fv[doc] = all_fv[doc]
...
def update(W, userFeedbacks, doc_M_rs, query_doc_N_dict, sample_episode, gamma, n_features, learning_rate, data):
