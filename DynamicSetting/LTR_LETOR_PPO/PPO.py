...
class ActorCritic(nn.Module):
    ...
    def evaluate_state(self, s):
        """Given states returns the state values"""
        probs = self.actor(s)
        phi_s=self.compute_phi(s.detach(),probs.detach())
        state_value = self.critic(phi_s)
...
class PPO:
    ...
    def update(self,counter,topK):
        """Update the policy and value network by optimizing the objective function"""

        #Get current query states,terminals
        curr_states=self.buffer.states[counter*topK:(counter*topK)+topK]
        curr_is_terminals=self.buffer.is_terminals[counter*topK:(counter*topK)+topK]
        curr_is_ep_trunc=self.buffer.is_ep_trunc[counter*topK:(counter*topK)+topK]
        curr_actions=self.buffer.actions[counter*topK:(counter*topK)+topK]
        curr_logprobs=self.buffer.logprobs[counter*topK:(counter*topK)+topK]
        curr_rewards=self.buffer.rewards

        # Monte Carlo estimate of returns
        rewards = []
        
        #bootstrap from final state in the buffer according to whether it is a terminal or not
        if curr_is_terminals[-1]:
           discounted_reward = 0
        else:
           discounted_reward = self.policy_old.evaluate_state(curr_states[-1]).item()
        
        #Run the buffers in reverse and bootstrap whenever an episode ends
        for reward,state,is_terminal,is_ep_trunc in zip(reversed(curr_rewards),reversed(curr_states),reversed(curr_is_terminals),reversed(curr_is_ep_trunc)):
            if is_terminal:
                discounted_reward = 0
            elif is_ep_trunc:
                discounted_reward= self.policy_old.evaluate_state(state).item()
        ...
        old_actions = torch.squeeze(torch.stack(curr_actions, dim=0)).detach().to(device)
        old_logprobs = torch.squeeze(torch.stack(curr_logprobs, dim=0)).detach().to(device)
    ...
    def save(self, checkpoint_path):
        """Save the current policy in the given path"""
