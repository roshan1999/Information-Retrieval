...
def transform(dataset, num_features):
    ...
    random.shuffle(QUERY_TRAIN)
    random.shuffle(QUERY_TEST)

    return QUERY_TRAIN, QUERY_VALI, QUERY_TEST, QUERY_DOC, QUERY_DOC_TRUTH, DOC_REPR, MAX_DCG

def normalize_features(DOC_REPR):
    mean = np.zeros(136)
    for doc in DOC_REPR:
        feature = np.asarray(DOC_REPR[doc])
        mean += feature
    mean = mean / len(DOC_REPR)
    sigma = np.zeros(136)
    for doc in DOC_REPR:
        feature = np.asarray(DOC_REPR[doc])
        sigma += (feature-mean)*(feature-mean)
    sigma = sigma/len(DOC_REPR)
    sigma = np.sqrt(sigma)
    ...
    for doc in DOC_REPR:
        feature = np.asarray(DOC_REPR[doc])
        DOC_REPR[doc] = (feature - mean) / sigma
...
def transform_all(dataset):
    ...
    DOC_REPR = normalize_features(DOC_REPR)

    random.shuffle(QUERY_TRAIN)
    random.shuffle(QUERY_TEST)

    return QUERY_TRAIN, None, QUERY_TEST, QUERY_DOC, QUERY_DOC_TRUTH, DOC_REPR, MAX_DCG
