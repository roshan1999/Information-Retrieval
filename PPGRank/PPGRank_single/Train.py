...
def train(validation_set, training_set, testset, theta, epochs, n_features, gamma, alpha, disp):
    ...
    for iterations in range(0, epochs):
        ...
        for Q in queries:
            ...
            if disp:
                print("Query = " + str(Q))
                validate_individual(data[Q], action_taken[Q], display=disp)
                print()
        ...
        # Calculates and returns training set's individual query NDCG scores and mean score
        print("\n\nValidating...: \n\n")
        results_validate = test(theta, validation_set, n_features, gamma, disp)
        ...
        print("\n\nTesting...")
        results_test = test(theta, testset, n_features, gamma, disp)
